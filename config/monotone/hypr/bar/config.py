...
class NightLightWidget(Box):
    def __init__(self, **kwargs):
        ...
        self.event_box.connect("scroll-event", self.on_scroll)
        self.add(self.event_box)
        self.update_nightlight()
        invoke_repeater(2000, self.update_nightlight)

    def on_scroll(self, _, event):
        step = 100
        if event.direction == 0:
            subprocess.run(["hyprctl", "hyprsunset", "temperature",f"+{step}%"])
        elif event.direction == 1:
            subprocess.run(["hyprctl", "hyprsunset", "temperature",f"-{step}%"])
        self.update_nightlight()
    def get_nightlight_status(self):
        try:
            output = subprocess.check_output(
    ...
    def update_nightlight(self):
        val=self.get_nightlight_status()
        self.icon_label.set_label(val)
...
#        self.system_tray = SystemTray(name="system-tray", spacing=4)
        ...
        self.nightlight_widget = NightLightWidget()
