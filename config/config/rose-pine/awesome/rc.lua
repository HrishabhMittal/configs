...
awful.spawn.with_shell("/home/hrishabhmittal/.config/awesome/lemonbar.sh &")
awful.spawn.with_shell("/home/hrishabhmittal/.config/awesome/batteryd &")

-- Rose Pine color palette
local colors = {
    base = "#191724",    -- Background
    surface = "#1f1d2e", -- Notification box
    text = "#e0def4",    -- Main text
    highlight = "#eb6f92", -- Title / Borders
    muted = "#908caa",   -- Subtitle text
}

-- Function to display error notifications
local function notify_error(title, message)
    naughty.notify({
        ...
        shape = function(cr, w, h)
            gears.shape.rounded_rect(cr, w, h, 10) -- Rounded corners
        end,
        opacity = 0.95,
        margin = 16,
        width = 400,
        icon = "/usr/share/icons/Papirus-Dark/32x32/status/dialog-error.svg", -- Change as needed
    })
end

-- Notify if there are startup errors
...
naughty.config.defaults.border_color = colors.highlight
naughty.config.defaults.bg = colors.surface
naughty.config.defaults.fg = colors.text
...
-- Define notification styles for different urgency levels
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey },            "b",     function () awful.spawn.with_shell("brave") end,
              {description = "runs browser (brave)", group = "Apps"}),
    awful.key({ modkey },            "f",     function () awful.spawn.with_shell("thunar") end,
...
local function set_random_wallpaper()
    local wallpaper_dir = "/home/hrishabhmittal/Pictures/rose-pine"
    local wallpapers = get_wallpapers(wallpaper_dir)
    if I == -1 then
        I = math.random(#wallpapers)
    end
    if #wallpapers > 0 then
        local random_wallpaper = wallpapers[I]
        I = (I % #wallpapers) + 1
